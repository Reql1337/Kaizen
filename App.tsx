...
const ProgressChart = React.lazy(() => import('./components/ProgressChart').then(m => ({ default: m.ProgressChart })));
const HistoryList = React.lazy(() => import('./components/HistoryList').then(m => ({ default: m.HistoryList })));
const Store = React.lazy(() => import('./components/Store').then(m => ({ default: m.Store })));
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (Capacitor.isNativePlatform()) {
      // Style Status Bar
      StatusBar.setStyle({ style: userState.theme === 'dark' ? Style.Dark : Style.Light });
      ...
      const backListener = CapApp.addListener('backButton', ({ canGoBack }) => {
        if (!canGoBack) {
          ...
          // You could implement custom back logic here if you had a router
          // For now, if we are in a sub-view, we go back to Home
  ...
  const handleOptionSelect = async (option: ActionOption) => {
    try { await Haptics.impact({ style: ImpactStyle.Light }); } catch (e) { }
  ...
  const handleActionComplete = async (ref: string, diff: DifficultyRating, mood: Mood) => {
  ...
  const toggleTheme = async () => {
    try { await Haptics.impact({ style: ImpactStyle.Light }); } catch (e) { }
    setUserState(prev => ({ ...prev, theme: prev.theme === 'light' ? 'dark' : 'light' }));
  };
  ...
  return (
    <Layout currentView={currentView} onChangeView={setCurrentView}>
      <React.Suspense fallback={<div className="h-full flex flex-col justify-center items-center animate-pulse"><Logo size="lg" /><p className="mt-4 text-primary font-serif">Loading...</p></div>}>
        ...
        {currentView === AppView.HOME && (
          <div className="h-full flex flex-col pt-safe-top">
            ...
            {!loadingAction && actionOptions.length > 0 && (
              <div className="flex-1 flex flex-col animate-slide-up"><h2 className="text-2xl font-serif text-text dark:text-textDark mb-4">Choose your path</h2><div className="space-y-4">{actionOptions.map((opt, i) => (<button key={i} onClick={() => handleOptionSelect(opt)} className="w-full bg-white/80 dark:bg-black/40 backdrop-blur-md p-6 rounded-3xl border border-white/60 dark:border-white/10 text-left hover:border-primary/50 transition-all duration-300 active:scale-95 shadow-lg shadow-black/5 hover:shadow-xl"><div className="text-[10px] uppercase font-bold text-stone-400 mb-2">Option {i + 1}</div><h3 className="text-xl font-bold text-text dark:text-textDark mb-2">{opt.title}</h3><p className="text-textLight dark:text-textLightDark mb-3 leading-relaxed">{opt.description}</p><span className="inline-block bg-stone-100/50 dark:bg-white/5 backdrop-blur-sm text-stone-500 dark:text-stone-400 text-[10px] px-2 py-1 rounded-lg font-bold">{opt.durationText}</span></button>))}</div></div>
            )}
        ...
        {currentView === AppView.SETTINGS && (
          <div className="animate-fade-in space-y-6 pt-safe-top pb-24">
            ...
            <div className="bg-white/80 dark:bg-black/40 backdrop-blur-xl p-5 rounded-[2rem] border border-white/60 dark:border-white/10 shadow-xl space-y-6">
              ...
              <div className="flex justify-between items-center border-b border-stone-50 dark:border-stone-800 pb-5">
                <span className="text-textLight dark:text-textLightDark text-sm">Dark Mode</span>
