...
export const DailyActionCard: React.FC<DailyActionCardProps> = ({ action, onComplete, onSkip, onRest, isMinimalist }) => {
  ...
  const containerClasses = isMinimalist
    ? "bg-white/90 dark:bg-surfaceDark/90 backdrop-blur-md rounded-2xl p-6 border-2 border-stone-100 dark:border-white/5 flex flex-col items-center text-center transition-all duration-300"
    : "bg-white/80 dark:bg-black/40 backdrop-blur-2xl rounded-[2.5rem] p-8 shadow-2xl shadow-stone-300/40 dark:shadow-black/50 border border-white/60 dark:border-white/10 flex flex-col items-center text-center transition-all duration-500 relative overflow-hidden";
      ...
      <div className="flex flex-col h-full justify-center animate-fade-in">
        <div className={containerClasses}>
          ...
          <div className="w-full space-y-3 relative z-10">
            ...
            <button
              onClick={onRest}
              className="w-full py-3 px-6 rounded-2xl font-sans font-medium text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 active:scale-95 transition-all duration-300"
            >
            ...
            <button
              onClick={onSkip}
              className="w-full py-2 text-sm text-stone-400 hover:text-stone-600 dark:hover:text-stone-300 active:scale-95 transition-all duration-300"
            >
              Skip (no guilt)
            </button>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 mb-8">
            {[
              ...
              <button
                ...
                className={`p-3 rounded-2xl border text-sm transition-all ${selectedDifficulty === opt.value
