...
export const Layout: React.FC<LayoutProps> = ({ children, currentView, onChangeView }) => {
  ...
  const handleNavClick = async (view: AppView) => {
    try {
  ...
  return (
    <div className="h-[100dvh] w-full bg-background dark:bg-backgroundDark text-text dark:text-textDark flex flex-col font-sans max-w-md mx-auto relative shadow-2xl transition-colors duration-300 overflow-hidden">
      ...
      {currentView !== AppView.ONBOARDING && currentView !== AppView.AUTH && (
        <nav className="absolute bottom-0 left-0 right-0 bg-white/70 dark:bg-black/40 backdrop-blur-2xl border-t border-gray-100 dark:border-white/10 p-4 flex justify-around items-center z-50 transition-colors duration-300 pb-[calc(1rem+env(safe-area-inset-bottom))]">
          <button
          ...
          <button
            onClick={() => handleNavClick(AppView.PROGRESS)}
          ...
          <button
            onClick={() => handleNavClick(AppView.HISTORY)}
          ...
          <button
            onClick={() => handleNavClick(AppView.STORE)}
